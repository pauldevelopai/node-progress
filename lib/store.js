...
const REPORTERS = "node_progress_tracker_reporters";
const ENTRIES   = "node_progress_tracker_entries";
const METRICS   = "node_progress_tracker_metrics";
...
export async function addReporter(host, { name, email, whatsapp, beat, daily_target } = {}) {
  ...
  await host.db.query(
    ...
    `INSERT INTO ${REPORTERS}
       (newsroom_id, reporter_key, name, email, whatsapp, beat, daily_target, active)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
    [key, clean, nn(email), nn(whatsapp), nn(beat), Number.isFinite(target) ? target : null, true]
  );
  return { ok: true, reporter: { reporter_key: key, name: clean }, existed: false };
}
