 ...
 * (node_progress_activity) + key/value store are created by the runtime;
...
export async function ensureSchema(pool) {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS node_progress_reporters (
    ...
    CREATE INDEX IF NOT EXISTS node_progress_reporters_nr
      ON node_progress_reporters (newsroom_id, reporter_key);
    -- added after the reporters table first shipped; safe + idempotent.
    ALTER TABLE node_progress_reporters ADD COLUMN IF NOT EXISTS submit_token text;

    CREATE TABLE IF NOT EXISTS node_progress_entries (
      id            bigserial PRIMARY KEY,
      newsroom_id   text NOT NULL,
      reporter_key  text,
      reporter_name text,
    ...
    CREATE INDEX IF NOT EXISTS node_progress_entries_nr
      ON node_progress_entries (newsroom_id, reporter_key);

    CREATE TABLE IF NOT EXISTS node_progress_metrics (
      id            bigserial PRIMARY KEY,
      newsroom_id   text NOT NULL,
      reporter_name text,
      channel       text,
    ...
    CREATE INDEX IF NOT EXISTS node_progress_metrics_nr
      ON node_progress_metrics (newsroom_id);
  `);
}
