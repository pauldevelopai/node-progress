...
import { CHANNELS, reporterKey } from "./store.js";
...
export function buildDashboard({ reporters = [], entries = [], metrics = [] } = {}, now = new Date()) {
  ...
  const reportersOut = [...buckets.values()].map((b) => {
    ...
    return { ...b, engagementRate: rate, targetStatus };
  });
