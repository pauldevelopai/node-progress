...
async function loadAll(host) {
  const [reporters, entries, metrics] = await Promise.all([
    listReporters(host), listEntries(host), listMetrics(host),
  ]);
  return { reporters, entries, metrics };
}

/** GET — the full manager accountability dashboard model. */
export async function getReport(host) {
  return buildDashboard(await loadAll(host), new Date(), host?.ctx?.newsroomId || "local");
}
...
export async function postBrief(host) {
  ...
  const d = buildDashboard(await loadAll(host));
  if (d.empty) throw new Error("Nothing to brief on yet — add reporters and log some output first.");

  const roster = d.reporters
    .map((r) => {
      const tgt = r.daily_target != null ? ` (target ${r.daily_target}/day, today ${r.today} → ${r.targetStatus})` : "";
      ...
      return `- ${r.name}${r.onRoster ? "" : " (not on roster)"}: ${r.week} items this week${tgt}; channels: ${ch}; last active ${r.lastActive || "never"}${perf}`;
    })
    .join("\n");
  ...
  const ctx =
    `WINDOW: this week starts ${d.weekStart}, today is ${d.today}.\n` +
    `TOTALS: ${d.topline.reporters} reporters, ${d.topline.activeThisWeek} active this week, ` +
    `${d.topline.itemsToday} items today, ${d.topline.itemsThisWeek} this week.\n` +
