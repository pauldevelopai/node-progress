...
export async function parseDailyReport(host, { text, reporterName, entryDate } = {}) {
  ...
  // Light newsroom grounding (host.profile, runtime >= v0.14.0): a country/audience
  // hint helps the model resolve local outlet/place names — disambiguation only,
  // never a source of invented items.
  const p = host.profile ? await host.profile.get() : null;
  const hint = p && (p.country || p.audience)
    ? `\nNEWSROOM CONTEXT (for disambiguation only — do NOT invent items from this): ` +
      [p.country && `country ${p.country}`, p.audience && `audience ${p.audience}`].filter(Boolean).join("; ") + `.`
    : "";

  const { text: reply, provider, model } = await host.ai.chat(
    PROMPT(body, reporterName, entryDate) + hint,
    { maxTokens: 1200 }
  );
